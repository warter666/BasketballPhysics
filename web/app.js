...
function shoot(a,p){
 attempts++;ui.attempts.textContent=attempts;const v=velocity(a,p);ball={...ballStart(),vx:v.vx,vy:v.vy,active:true,path:[],collisions:0,t:0};
...
function finish(scored,reason){
 ball.active=false;ball=null;preview=[];if(scored)made++;
...
function drawHoop(){
 ...
 ctx.strokeStyle="#d8e1e8";ctx.lineWidth=2;for(let i=-35;i<=35;i+=18){ctx.beginPath();ctx.moveTo(h.x+i,h.y+3);ctx.lineTo(h.x+i*.65,h.y+42);ctx.stroke()}ctx.beginPath();ctx.moveTo(h.x-24,h.y+42);ctx.lineTo(h.x+24,h.y+42);ctx.stroke();
}
...
function drawPreview(a,p){
 ...
 const sim=simulate(velocity(a,p));ctx.fillStyle="#6fc1ff";for(let i=0;i<sim.path.length;i+=6){const q=sim.path[i];ctx.beginPath();ctx.arc(q.x,q.y,2.2,0,Math.PI*2);ctx.fill()}
}
function drawFlight(){
